...
def create_and_store_embeddings(
    ...
    #vector_store.persist() # Not strictly needed here but good practice if adding incrementally later
...
def process_data(force_reprocess: bool = False):
    ...
    if os.path.exists(CHROMA_DB_PATH) and not force_reprocess:
        ...
        try:
            ...
            logging.warning(
                f"ChromaDB directory exists, but collections might be missing or invalid ({e}). Reprocessing...")
    ...
    try:
        ...
        logging.error(
            "Please ensure sentence-transformers and necessary model dependencies (like PyTorch or TensorFlow) are installed.")
